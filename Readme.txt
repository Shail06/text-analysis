Installing the Project
----------------------
1) Install the VirtualEnvironment with Python 3 and pip
2) pip install -r requirements.txt   // This will install the required packages 
3) Open the Python prompt and do the following:
    import nltk
    nlkt.download()
    // Download the following packages: stopwords, wordnet, punkt,  maxent_ne_chunker, averaged_perceptron_tagger
4) Move out of python promt and run the project as:
    python manage.py runserver
    
    
